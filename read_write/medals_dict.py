...
#
# with open(file_name,'w',encoding='utf-8',newline='') as outputfile:
...
############ to sort the dictionary on above code
# def sort_key(d):
#     return d['country']
# columns = ['country', 'gold','silver','bronze','rank']
#
...
######### Remove row from the dict #############
columns = ['country', 'gold','silver','bronze']

file_name = 'country_medals.csv'

with open(file_name,'w',encoding='utf-8',newline='') as outputfile:
    writer = csv.DictWriter(outputfile,fieldnames=columns,extrasaction='ignore')
    writer.writeheader()
    # for row in medals_table:
    #     writer.writerow(row)
    writer.writerows(medals_table)